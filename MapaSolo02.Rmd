---
title: "Seguimiento CNPV 2018 (`r format(Sys.time(), '%d de %B de %Y')`)"
output: 
...
#str(reqconal)
#reqconal$DIVIPOLA<-ifelse(nchar(reqconal$Codigo.del.municipio) == 5,reqconal$Codigo.del.municipio, paste0(reqconal$Código.departamento,"001"))
...
#Inicio
MunF2<-read.table("E:/R/CONAL/MunicipiosFaseII.csv",header=TRUE,sep=";",stringsAsFactors=FALSE,encoding = "UTF-8")
MunF3<-read.table("E:/R/CONAL/MUNICIPIOSFASEIII.csv",header=TRUE,sep=";",stringsAsFactors=FALSE,encoding = "UTF-8")
names(MunF3)<-names(MunF2)
...
FAS<-readOGR(dsn="C:/Geo",layer="FASES",stringsAsFactors=FALSE,use_iconv=TRUE,encoding="UTF-8")
#str(DPTOS@data)
...
AOL<-readOGR(dsn="E:/R/CONAL",layer="AOL201806nn",stringsAsFactors=FALSE,use_iconv=TRUE,encoding="UTF-8")
...
MPIOSIni$INICIO<-ifelse((MPIOSIni$inicio == "NO") | is.na(MPIOSIni$inicio),"No","Si")
...
bins <-c(0,0.000001,90,100,max(Trans@data$Porcentaje))
tr.pal <- colorBin( "RdYlBu", bins=bins, na.color = "#bababa")


#factpal2 <- c("#e0e0e0","#fee08b","#f46d43","#66bd63")
#palData <- classIntervals(Trans$Porcentaje, n=4, style="fixed",fixedBreaks=c(0,0.000001,90,100,max(Trans@data$Porcentaje,na.rm = TRUE)))
#Trans$colors <- findColours(palData, factpal2)


map<-leaflet(data= AOL) %>%
  ...
  ## Overlay groups
  addMarkers(clusterOptions = markerClusterOptions(),group = "Requerimientos CONAL")%>%
  addCircleMarkers(data=CAPITALES,radius = 2.2 ,stroke = FALSE, fillOpacity = 1, color = "#525252",group = "Capital departamental") %>%
   addPolygons(data = MPIOSIni, stroke = FALSE, fillOpacity = 0.5, fillColor = ~factpal(INICIO),group = "Inicio Operación")%>%
  addPolygons(data = Trans, stroke = FALSE, fillOpacity = 0.4,group = "Transmisión", fillColor = ~tr.pal(Porcentaje)) %>%
 addPolylines(data = DPTOS_lin, stroke = TRUE, color = "#000000",opacity = 1, weight= 1,group = "Límite departamental") %>%

  ## Layers control
  addLayersControl(overlayGroups = c("Requerimientos CONAL","Inicio Operación","Transmisión"), options = layersControlOptions(collapsed = FALSE))%>%

  # Add legend
  addLegend(pal = factpal, values = MPIOSIni$INICIO, opacity = 0.5, title = 'Estado   inicio',position = "bottomleft")%>%
  addLegend(position = "bottomleft", pal = tr.pal, labels=c("0%","0.01 - 90%","90-100%","Mayor al 100%"), values = Trans$Porcentaje,title = "Transmisión",opacity = 0.3)
  #addLegend(pal=tr.pal, values= Trans$Porcentaje)
